...
def main(
    ...
    t, y = get_data_diffeq(dataset_size, key=data_key)
    ...
    def PINN_loss(model, t, y):
        ...
        pred_y = model(t)
        pred_y_t0 = model(jnp.array([0]))
        #pred_y_2tpi = model(jnp.array([jnp.pi*2]))

        def periodic_loss():
            # minimise d2y/dt2 + dy/dt + y
            dydt = jax.jacrev(model, argnums = 0)(pred_y)
            d2ydt2 = jax.jacrev(model, argnums = 0)(dydt)
            return d2ydt2 + dydt + pred_y # add constants
    ...
    t_test = jnp.linspace(0, 6 * jnp.pi, 100)
    for ts in t_test:
    ...
    plt.plot(res_t, diffeq(t_test, (2*jnp.pi)),0.1,0)
    
    plt.xlabel('t')
    plt.ylabel('y')
    plt.savefig('feedforwardNN/Results/resultsPINN2ode.png')
    plt.show()
