...
def test_sine_x():
  ...
  def gradient_fn(forward_fn, loss_outputs, initial_data): # forward_fn is the forward pass function
    ...
    def model_loss(params, target, weights, rng, x_train): # params is the matrix of weights and biases
      ...
      def periodic_loss(params, x):
        ...
        return u_x + jnp.sin(x) # expression to minimise (dx/dt + sin(x))
  ...
  # consider replacing fixed values with variables which correspond to the test range so physics is
  # informed over the whole data range
  #in_array = np.linspace(-1 * np.pi, 1 * np.pi, 5)
  in_array = jnp.array([0])
  ...
  dataset = NumpyDataset(X_f, jnp.cos(X_f))
