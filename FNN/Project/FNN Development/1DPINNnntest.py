...
def get_data_difeq(dataset_size, *, key):
    ...
    Wn = 1/(2 * jnp.pi) # natural frequency
...
def main(
    ...
    t, y = get_data_difeq(dataset_size, key=data_key)
    ...
    t_test = jnp.linspace(0, 4 * jnp.pi, 100)
    for ts in t_test:
    ...
    plt.xlabel('t')
    plt.ylabel('y')
    plt.savefig('feedforwardNN/Results/resultsPINNode.png')
    plt.show()
