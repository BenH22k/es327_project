...
class FNN(eqx.Module):
    ...
    #     # def f(carry, inp, i):
    #     #     return self.layers[i](inp, carry), None

    #     # for i, layer in enumerate(self.layers[:-1]):
    #     #     x, _ = jax.lax.scan(f, hidden, x, i)
    #     x_list = []
    #     out = []
    #     for x in x_batch:
    #         x = jnp.array([x])
    ...
    #             # x = jax.vmap(layer)(jnp.array([jnp.array([x])]))
    #             # x = jax.vmap(jax.nn.tanh)(x)
    #         out.append(self.layers[-1](x) + self.bias)
    #         #x_list.append(x)

    #     #out = jax.vmap(self.layers[-1])(jnp.array(x_list))
    ...
    #         inp = jnp.array([inp])
    #         for layer in self.layers[:-1]:
    #             #inp = jax.nn.tanh(layer(inp))
    #             inp = (layer(inp))
    #         #return self.layers[-1](inp,carry), None
    #         s = self.layers[-1](inp)
    #         return s, None

    #         #return self.layers(inp, carry), None

    #     out, _ = jax.lax.scan(f, hidden, input, 30)
    #     print(out)
    #     return jax.nn.tanh(self.layers[-1](out) + self.bias)
...
def main(
    ...
    model = FNN(in_size=1, out_size=1, hidden_size=10, key=model_key)
    ...
    def PINN_loss(model, t, y):
        ...
        #pred_y = model(t)
        # need to use vmap to allow batching:
    ...
    def make_step(model, t, y, opt_state):
        f_loss, u_loss = PINN_loss(model, t, y)
        #updates, opt_state = optim.update(grads, opt_state)
        #model = eqx.apply_updates(model, updates)
        return f_loss, u_loss, #model, opt_state
    ...
    for step, (t, y) in zip(range(steps), iter_data):
        t = jnp.array([t])
        y = jnp.array([y])
        f_loss, u_loss, = make_step(model, t, y, opt_state)
        loss = f_loss + u_loss
        loss = loss.item()
        print(f"step={step}, floss={f_loss}, uloss={u_loss}")
    ...
    plt.xlabel('t')
    plt.ylabel('y')
    plt.savefig('feedforwardNN/Results/resultsPINNb.png')
    plt.show()
