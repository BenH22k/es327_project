...
class FNN(eqx.Module):
    ...
    #     #     #x = jnp.array([x])
    #     #     for layer in self.layers[:-1]:
    #     #         x = jax.nn.tanh(layer(jnp.expand_dims(x,0)))
    #     #     x = self.layers[-1](x[1]) + self.bias
    #     #     out.append(x)
        
            
    #     # return jnp.squeeze(jnp.array(out),1)
...
#):
    ...
    def PINN_loss(model, t, y, consts):
        ...
        def periodic_loss(pred_y):
            ...
            if jnp.ndim(loss_params) == 1:
                out = loss_eqn(loss_params)
            else:            
